...
df = pd.read_csv('estatística/dados_estudantes.csv', sep=';', decimal='.', encoding='utf-8-sig')

# Declarar a Figura do Gráfico
...
# Caso 2: Quaartis - Outliers de Valor de bolsas
...
print(f"4. TEMPO DAS ENTREGA (Corte: > {CORTE_P95:.2f})")
...
print(f"6. Doses de Radiacao (Corte: > {corte_p75:.2f})")
print(df[df['Dose_Radiacao'] >= corte_p75][['Matricula', 'Nome', 'Dose_Radiacao']].to_string(index=False))
plt.subplot(1, 6, 6)
plt.title('Dose de Radiacao')
plt.ylabel('Radiacao (uSv)')
