...
# 4. Número das Classes
k_sturges = 1 + 3.322 * np.log10(n)
k = round(k_sturges)
print(f"4. Numero de classes (K): {k} \n")
...
# 7. Tabela de Frequencia
frequencia_abs = pd.cut(rol, bins=bins, include_lowest=True, right=False). value_counts()
print(f"Frequecias absolutas : \n {frequencia_abs} \n")
# 8. Grafico - Histograma
plt.figure(figsize=(10, 6))
plt.hist(rol, bins=bins_plot, density=False)
plt.title('Histograma da Distribuição de Frquência')
