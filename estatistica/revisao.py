...
#1. carregar dados
df = pd.read_csv('C:/Users/EFG/TEC-SLLKO-CD-N1-2025/estatistica/notas.csv')
...
#3. media simples por aluno
media_simples_alunos = df [['B1', 'B2', 'B3', 'B4']].mean(axis=1)
#4. media ponderada por aluno
pesos = [1, 2, 3, 4]
media_ponderada_alunos = (df['B1']*pesos[0] + df['B2']*pesos[1] + df['B3']*pesos[2] + df['B4']*pesos[3] /sum(pesos))
...
plt.subplot(1, 2, 1)
sns.histplot(todas_notas, color='red', alpha=0.7)
plt.xticks(np.arange(0,11,1))
plt.title('Histograma de frêquencia das notas')
plt.xlabel('Notas')
plt.ylabel('Frequencia')

# boxplot
plt.subplot(1, 2, 2)
sns.boxplot(y=todas_notas, color='pink')
