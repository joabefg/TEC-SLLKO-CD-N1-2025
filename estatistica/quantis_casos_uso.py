...
sns.boxplot(y=df['Hemoglobina'], color='red')

# Caso 6: radiação - (curso de radiologia - 75% maiores doses)
corte_q75 = df['Dose_Radiacao'].quantile(0.75)
print(f'6. Doses de radiação (Corte: > {corte_q75:.2f})')
print(df[df['Dose_Radiacao'] >= corte_q75][['Matricula', 'Nome', 'Dose_Radiacao']].to_string(index=False))
plt.subplot(1, 6, 6)
plt.title('Dose de Radiação')
plt.ylabel('radiação (uSv)')
sns.boxplot(y=df['Dose_Radiacao'], color='purple')
